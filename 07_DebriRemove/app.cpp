...
static const float TARGET_ANGL[] = { 0, 45, 135, };
static const float TARGET_DIST[] = { 680, 250, };

debrisys::ColorExtract::HSV lowerBlue{  95,   0,   0 };     // 青色HSV下限値
debrisys::ColorExtract::HSV upperBlue{ 105, 255, 255 };     // 青色HSV上限値
...
debrisys::Camera*       gBottleImg;     // ボトル画像
debrisys::ColorExtract* gBlueExt;       // 青色抽出
debrisys::ExtractRange* gBottleLimit;   // ボトル限定
debrisys::ColorDetect*  gBlueDetect;    // 青ボトル検出
...
static void create_system()
{
    ...
    gBottleImg  = new debrisys::Camera();
    gBlueExt    = new debrisys::ColorExtract(lowerBlue, upperBlue);
    gBottleLimit= new debrisys::ExtractRange();
    gBlueDetect = new debrisys::ColorDetect(gBottleImg, gBlueExt, gBottleLimit);
    ...
    /* 旋回角度判定 */
    gRotateJudge[0] = new debrisys::RotateAngleJudge(gJudgeAng[0], TARGET[0]);
    gRotateJudge[1] = new debrisys::RotateAngleJudge(gJudgeAng[1], TARGET[1]);
    gRotateJudge[2] = new debrisys::RotateAngleJudge(gJudgeAng[2], TARGET[2]);
    gRotateJudge[3] = new debrisys::RotateAngleJudge(gJudgeAng[3], TARGET[1]);
    gRotateJudge[4] = new debrisys::RotateAngleJudge(gJudgeAng[4], TARGET[2]);
    gRotateJudge[5] = new debrisys::RotateAngleJudge(gJudgeAng[5], TARGET[1]);
    gRotateJudge[6] = new debrisys::RotateAngleJudge(gJudgeAng[6], TARGET[2]);
    gRotateJudge[7] = new debrisys::RotateAngleJudge(gJudgeAng[7], TARGET[1]);
    gRotateJudge[8] = new debrisys::RotateAngleJudge(gJudgeAng[8], TARGET[2]);
    ...
    gAreaStep = new debrisys::Step[2] {
        (gLeftRotate[0], gRotateJudge[0], gInit[0]),    // 手順1
        (gFrontStra[0], gDistJudge[0], gInit[1])        // 手順2
    };
    gCir1Step = new debrisys::Step[4] {
        (gLeftRotate[1], gRotateJudge[1], gInit[2]),    // 手順1
        (gFrontStra[1], gDistJudge[1], gInit[3]),       // 手順2
        (gBackStra[0], gDistJudge[2], gInit[4]),        // 手順3
        (gRightRotate[0], gRotateJudge[2], gInit[5])    // 手順4
    };
    gCir2Step = new debrisys::Step[4] {
        (gLeftRotate[2], gRotateJudge[3], gInit[6]),    // 手順1
        (gFrontStra[2], gDistJudge[3], gInit[7]),       // 手順2
        (gBackStra[1], gDistJudge[4], gInit[8]),        // 手順3
        (gRightRotate[1], gRotateJudge[4], gInit[9])    // 手順4
    };
    gCir3Step = new debrisys::Step[4] {
        (gRightRotate[2], gRotateJudge[5], gInit[10]),  // 手順1
        (gFrontStra[3], gDistJudge[5], gInit[11]),      // 手順2
        (gBackStra[2], gDistJudge[6], gInit[12]),       // 手順3
        (gLeftRotate[3], gRotateJudge[6], gInit[13])    // 手順4
    };
    gCir4Step = new debrisys::Step[4] {
        (gRightRotate[3], gRotateJudge[7], gInit[14]),  // 手順1
        (gFrontStra[4], gDistJudge[7], gInit[15]),      // 手順2
        (gBackStra[3], gDistJudge[8], gInit[16]),       // 手順3
        (gLeftRotate[4], gRotateJudge[8], gInit[17])    // 手順4
    };

    gCircle = new debrisys::Circle[4] {
        (gCir1Step, gBlueDetect, true),
        (gCir2Step, gBlueDetect, true),
        (gCir3Step, gBlueDetect, true),
        (gCir4Step, gBlueDetect, true)
    };

    gBtoA   = new debrisys::AreaMove(gAreaStep);
    gAarea  = new debrisys::DebriRemove(gCircle);
    gSection= new debrisys::Section[2] { (gBtoA), (gAarea) };
...
void camThread()
{
    gBottleImg->getImage();     // カメラから画像を取得する
}
