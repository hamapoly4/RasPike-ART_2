...
debrisys::SubRun* gRightRotate = nullptr;   // 右旋回走行
debrisys::SubRun* gFrontStra   = nullptr;   // 前進走行

debrisys::Judge* gRightJudge = nullptr; // 右旋回判定
...
static void create_system()
{
    ...
    gRightRotate    = new debrisys::RotateRun(gLeftMotor, gRightMotor, gGyroTurn, gEncoderTurn[0], debrisys::ERotDire::RIGHT);
    gFrontStra      = new debrisys::StraightRun(gLeftMotor, gRightMotor, gEncoderTurn[1], debrisys::EStraDire::FRONT);
    gRightJudge     = new debrisys::RotateAngleJudge(gJudgeAng, TARGET[0]);
    ...
    gStep[0]        = new debrisys::Step(gRightRotate, gRightJudge, gInit[0]);
