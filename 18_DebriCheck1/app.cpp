...
void* cameraThread(void* arg) {
    ...
    Mat hsv_image;
    Mat mask_image;
    Scalar lowerBlue(H_MIN, S_MIN, V_MIN);
    Scalar upperBlue(H_MAX, S_MAX, V_MAX);
    ...
    while(true) {
		...
		if (pressed != HUB_BUTTON_LEFT) {
			...
			// inRangeを用いてフィルタリング
			inRange(hsv_image, lowerBlue, upperBlue, mask_image);
			imshow("mask", mask_image);
			waitKey(1);
			
			roiFrame = mask_image(roi);
			imshow("roiFrame", roiFrame);
			waitKey(1);
			
			blue_data = countNonZero(roiFrame);
